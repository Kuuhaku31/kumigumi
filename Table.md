...
本文档描述当前 SQLite schema。字段、主键和外键关系应与 `src/main/java/Database/SQLiteSQL.java`、`src/main/java/Database/SQLiteAccess.java` 以及 `Info.*Info` 类保持一致。

统一约定：

- 表名、字段名和 Java `Info.*Info` 类型保持一致。
