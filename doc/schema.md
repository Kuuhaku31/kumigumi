...
- 单条 Info 写入失败不会中断后续数据；处理结束后由用户决定提交成功项或回滚整个事务。
...
该顺序保证 `episode`、`episode_record`、`rss`、`torrent_page` 的外键依赖尽量先满足。单条数据的 `SQLException` 会被记录并继续处理；全部写入结束后由用户决定提交成功项或 rollback。连接、SQL 准备等无法继续执行的异常会直接 rollback。
...
`required_rss` 保存 `view_torrent_page` 需要查询的 RSS URL，由 `Utils.CreateDatabaseViews` 从 RSS URL 文件读取后整表替换。

| 字段      | 类型 | 必需 | 说明                         |
| --------- | ---- | ---- | ---------------------------- |
| `URL_RSS` | text | 是   | torrent 页面允许使用的 RSS URL |

主键：`URL_RSS DESC`

## view 表格

以下视图由 `SQLiteAccess` 初始化和维护。`Utils.CreateDatabaseViews` 从两个文件读取筛选条件，并在同一个事务中整表替换 `required_anime_id` 和 `required_rss`：

```powershell
java --enable-native-access=ALL-UNNAMED `
  -cp target/kumigumi-<version>.jar `
  Utils.CreateDatabaseViews <database-path> <ani-id-file> <rss-url-file>
```

ANI_ID 文件可使用换行、空白、逗号或分号分隔整数。RSS URL 文件每行保存一个 URL，空行会被忽略。两个文件中的重复值都会被自动去除。

`view_anime` 和 `view_episode` 只通过 `required_anime_id` 过滤。`view_torrent_page` 使用交集条件：关联的 `ANI_ID` 必须存在于 `required_anime_id`，同时 `URL_RSS` 必须存在于 `required_rss`。任一筛选表为空时，依赖该表的视图结果为空；更新筛选表后无需重新创建视图。
...
筛选条件：`rss.ANI_ID` 命中 `required_anime_id` 且 `torrent_page.URL_RSS` 命中 `required_rss`。
