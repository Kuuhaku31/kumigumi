...
def 数据处理(
    动画信息列表: list[dict], 单集信息列表: list[dict], 种子信息列表: list[dict]
) -> tuple[list[list[str]], list[list[str]], list[list[str]]]:
    # 翻译
    动画信息列表 = [{headers.字段字典.get(k, k): v for k, v in row.items()} for row in 动画信息列表]
    单集信息列表 = [{headers.字段字典.get(k, k): v for k, v in row.items()} for row in 单集信息列表]
    种子信息列表 = [{headers.字段字典.get(k, k): v for k, v in row.items()} for row in 种子信息列表]
