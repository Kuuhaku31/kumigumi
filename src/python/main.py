...
class 工作表数据:
    def __init__(己):
        己.主键: str = ""
        己.表头_不含主键: list[str] = []
        己.数据: list[dict[str, str]] = []


def 获取工作表数据(work_book: Workbook, 工作表名: str) -> 工作表数据:
    ...
    # 翻译
    工作表数据实例.主键 = 主键
    工作表数据实例.表头_不含主键 = [k for k in 字段字典.keys() if k != 主键]

    # 读取数据区域
    for 行号 in range(起始行, 结束行):
        row_data: dict[str, str] = {}
        for 字段名, 列号 in 字段字典.items():
            单元格值 = 工作表.cell(row=行号, column=列号).value
            row_data[字段名] = 单元格值 if 单元格值 is not None else ""
        工作表数据实例.数据.append(row_data)
...
if __name__ == "__main__":
    ...
    for 数据库表名, 工作表名 in res.A:
        data: 工作表数据 = 获取工作表数据(work_book, 工作表名)
        # 翻译
        data.数据 = [{headers.字段字典.get(k, k): v for k, v in row.items()} for row in data.数据]
        data.主键 = headers.字段字典.get(data.主键, data.主键)
        data.表头_不含主键 = [headers.字段字典.get(k, k) for k in data.表头_不含主键]

        更新数据库(data.数据, data.主键, data.表头_不含主键, res.数据库地址, 数据库表名)
