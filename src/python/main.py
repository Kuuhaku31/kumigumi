...
def 更新Access数据库(
    ...
    数据库episode表名: str,
    数据库torrent表名: str,
    anime_sheet_store_list,
    episode_sheet_store_list,
    torrent_sheet_store_list,
):
    # 更新 Access 数据库
...
def 批量获取远程数据并更新数据库(work_book: Workbook, anime_sheet_fetch_list: list[str]) -> None:
    # 批量获取远程数据并更新数据库
...
def 下载种子(work_book: Workbook, torrent_download_sheet_name: str) -> None:
    # 下载种子链接
...
if __name__ == "__main__":
    ...
    批量获取远程数据并更新数据库(work_book, anime_sheet_fetch_list)
    下载种子(work_book, torrent_download_sheet_name)
