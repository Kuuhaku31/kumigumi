...
```text
Excel 指令/数据块 -> Task.* 抓取任务 -> Database.*Info 数据对象 -> SQLiteAccess.UpsertInfo -> SQLite
```
...
- 使用 SQLite 保存结构化数据，schema 以 `Table.md`、`Database.*Info`、`SQLiteSQL` 和 `SQLiteAccess` 初始化逻辑为准。
